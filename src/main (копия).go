...
var urlApi = "https://api.coinmarketcap.com/v1/ticker/?limit=500"
...
func main() {
	...
	bs, err := ioutil.ReadFile("/home/sas/golang/src/pricefromcoinmarketcup/src/currencyold.txt") //Считываем с файла перечень валлют, которыми торгуем
	if err != nil {
		log.Fatal(err)
	}
	...
	logs.Info(len(currencyListInFile))
